...
export function DeleteProcessModal(props: any) {
    const router = useRouter()
  const cancelButtonRef = useRef(null)

  function deleteProcess(){
    props.setOpen(false)
    set(ref(db, 'processes/' + props.process.ProcessId), {});
      router.reload()
  }
          ...
          >
            <div className="relative inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Wollen Sie den Stickvorgang "{props.process.Name}" vom {props.process.LastChangeTime} entgültig löschen?
                    </p>
                  </div>
                </div>
              </div>
              <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                <button
                  type="button"
                  className="w-full inline-flex justify-center rounded-none border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm"
